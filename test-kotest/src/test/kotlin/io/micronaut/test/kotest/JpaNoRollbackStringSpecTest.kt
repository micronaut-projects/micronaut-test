...
@DbProperties
class JpaNoRollbackStringSpecTest(private val entityManager: EntityManager,
                                  private val transactionManager: PlatformTransactionManager) : StringSpec() {
    init {
        "given: no rollback between tests, when: test persist one, then: the book is persist " {
            val book = Book()
        ...
        "given: a new transaction, when: test persist two, then: the book is persist" {
            val book = Book()
            ...
            entityManager.find(Book::class.java, book.id) shouldNotBe null
                ...
                val query = entityManager.criteriaBuilder.createQuery(Book::class.java)
                query.from(Book::class.java)
                entityManager.createQuery(query).resultList shouldHaveSize 2
        }
