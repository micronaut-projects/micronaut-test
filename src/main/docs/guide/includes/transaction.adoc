When using `@MicronautTest` each `@Test` method will be wrapped in a transaction that will be rolled back when the test finishes. This behavior can be changed by using the `transactional` and `rollback` properties.
...
To not roll back the transaction:
