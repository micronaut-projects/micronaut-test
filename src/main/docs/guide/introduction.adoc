...
** `micronaut.test.extensions.junit5.annotation.MicronautTest` for <<python,Python tests>>, which are compiled to JUnit 5 tests.
...
snippet::io.micronaut.test.junit5.EagerInitializationTest[project="test-junit5", language="java", tags="eager", indent=0, title="Using an HttpClient in a test with eager singleton initialization enabled"]
