...
[source,java]
----
include::{junit5tests}/resource/MongoResource.java[]
----

<1> Define a class that implements api:test.support.resource.TestResource[
<2> In the `start` method start the container
<3> Return the configuration that should apply to the application (in this case the `mongodb.uri` configuration option)
